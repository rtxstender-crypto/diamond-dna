...
describe("Ask DiamondDNA routing and deterministic grounding",()=>{
  it("routes MLB, MiLB, Gem Score, history, similarity, and unsupported intents",()=>{
    ...
    expect(routeAssistantIntent("What date did he hit for the cycle?")).toBe("game-history");
  });
  ...
  it("never fabricates missing game-level or Statcast answers",()=>{expect(buildDeterministicAnswer(mlbContext,"game-history","best game").answer).toContain("will not answer this game-level question from model memory");expect(buildDeterministicAnswer(mlbContext,"unsupported-statcast","exit velocity").answer).toContain("doesn't currently have verified Statcast")});
